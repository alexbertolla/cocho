...
if __name__ == '__main__':
    ...
    th_barriga.start()
    th_barriga.join(timeout=3)
    contador_erro = 1

    try:
        while th_barriga.is_alive():
            if not cam_barriga.running or contador_erro <= 3:
                contador_erro += 1
                pass
            elif cam_barriga.running:
                frame_captured = cam_barriga.frame_show
                cv.imshow(f"{cam_barriga.get_serial_number}", frame_captured)
